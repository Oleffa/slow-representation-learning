...
def get_label(data, label, dt, device):
    dt = dt.long()
    d = (data[label][:,:,[0,1,4]]+350.0) / (2 * 350.0)
    labels = []
...
def plot_batch(o_in, o_pred, num=16, device='cpu'):
    ...
    comp = torch.hstack([o_in.view(o_in.shape[0] * o_in.shape[1], o_in.shape[2], o_in.shape[3]), \
            o_pred.view(o_pred.shape[0] * o_pred.shape[1], o_pred.shape[2], o_pred.shape[3])])
